...
function getValuesForm() {
  ...
  for (let i of form) {
    if (
      i.id !== "pickup" &&
      i.id !== "courier_deliver" &&
      i.className !== "stores_for_pickup" &&
      i.className !== "input_item_active" &&
      i.className !== "input_item_active_num_paycard" &&
      i.name !== "payment_method" &&
      i.className !== "input_item_active_CVV_paycard" &&
      i.className !== "info_for_courier_deliver" &&
      i.className !== "input_item_active_date_paycard" &&
      i.className !== "button_order" &&
      i.className !== "paycard"
    ) {
    ...
    .then((data) => {
      if (data.error) {
        ...
        if (document.querySelector("#cash").checked) {
          ...
          happy.innerHTML = `<h1 style='color:black; font-family: SF Pro Text;font-size:30px;font-weight:500;'>Заказ №${
            data.order_number
          } оформлен, оплата при получение заказа</h1>
          ...
          happy.innerHTML = `<h1 style='color:black; font-family: SF Pro Text;font-size:30px;font-weight:500;'>Заказ №${
            data.order_number
          } оформлен, чек отправлен на email:${data.user_email}</h1>
                            <div style='display:flex; align-items:center; justify-content: center; margin-top: 40px'>
                            <a href='${localStorage.getItem("baseUrl")}/catalog/'
                            style='
                            border-radius: 4px;
